...
namespace PowerLanguage.Strategy
{
    ...
    public class click_trade_strategy : SignalObject
    {
        ...
        // Visual line for order price
        private ITrendLineObject m_OrderLine = null;
            ...
            if (StrategyInfo.MarketPosition == 0)
            {
                ...
                if (m_BuyOrderActive && m_TargetPrice > 0)
                {
                    try
                    {
                        ...
                        if (Bars.CurrentBar % 10 == 0)
                        {
                            Output.WriteLine("[LIVE] Buy Stop-Limit active @ Stop: " + m_TargetPrice.ToString("F2") +
                                           " Limit: " + limitPrice.ToString("F2") +
                                           " | Current: " + Bars.Close[0].ToString("F2"));
                        }
                    }
                    catch (Exception ex)
                    {
                ...
                if (m_SellOrderActive && m_TargetPrice > 0)
                {
                    try
                    {
                        ...
                        if (Bars.CurrentBar % 10 == 0)
                        {
                            Output.WriteLine("[LIVE] Sell Stop-Limit active @ Stop: " + m_TargetPrice.ToString("F2") +
                                           " Limit: " + limitPrice.ToString("F2") +
                                           " | Current: " + Bars.Close[0].ToString("F2"));
                        }
                    }
        ...
        protected override void OnMouseEvent(MouseClickArgs arg)
        {
            ...
            if (arg.buttons == MouseButtons.Left && (arg.keys & Keys.Shift) == Keys.Shift)
            {
                ...
                if (clickPrice > currentPrice)
                {
                    ...
                    DrawOrderLine(m_TargetPrice, Color.Green);
                ...
                else if (clickPrice < currentPrice)
                {
                    ...
                    DrawOrderLine(m_TargetPrice, Color.Red);
        ...
        private void DrawOrderLine(double price, Color lineColor)
        {
            ...
            try
            {
                // Create horizontal line at order price
                DateTime currentTime = Bars.Time[0];
                DateTime endTime = currentTime.AddMinutes(60);

                ChartPoint begin = new ChartPoint(currentTime, price);
                ChartPoint end = new ChartPoint(endTime, price);

                m_OrderLine = DrwTrendLine.Create(begin, end);
                m_OrderLine.Color = lineColor;
                m_OrderLine.ExtLeft = true;
                m_OrderLine.ExtRight = true;
                m_OrderLine.Size = 2;
            }
            catch (Exception ex)
            {
